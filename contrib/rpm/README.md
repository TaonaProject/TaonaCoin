...
The RPM spec file provided here is for Taona-Core 0.12.0 and builds on CentOS
...
reference the release tarball available from https://taona.org/bin/ and
`Source1` should reference the BerkeleyDB source.

Sources 10-99 are for source files that are maintained in the
[Taona git repository](https://github.com/TaonaProject/Taonacoin) but are not part of
...
Currently only `Source10` is used, for the example `taona.conf` file.
...
in the taona git repository. At present only an SVG version of the Taona
icon is used.

## Patches

In general, patches should be avoided. When a packager feels a patch is
necessary, the packager should bring the problem to the attention of the taona
...
needed when 0.12.1 is released, a proper fix is already in the Taona git
master branch.
...
idea to build Taona Core against a static version of that library compiled
...
The `taona.svg` file is from the source listed as `Source100`. It is used as
...
matters for the `taona-server` package. At this point, most RPM based
...
The `taona-server` package should have SELinux support. How to properly do
...
Taona-Core against OpenSSL or use a patched version of Boost in the build
system.
...
[Ringing Liberty](https://www.ringingliberty.com/taona/). He has been
packaging Taona for Fedora at least since 2012.
...
4. I renamed the `taona` package that contains the Qt GUI to `taona-core` as
that appears to be how the general population refers to it, in contrast to
`taona-xt` or `taona-classic`. I wanted to make sure the general population

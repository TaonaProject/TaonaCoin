...
#include "taonaaddressvalidator.h"
...
void setupAddressWidget(QValidatedLineEdit *widget, QWidget *parent)
{
    ...
    widget->setPlaceholderText(QObject::tr("Enter a Taona address (e.g. %1)").arg(
        QString::fromStdString(DummyAddress(Params()))));
#endif
    widget->setValidator(new TaonaAddressEntryValidator(parent));
    widget->setCheckValidator(new TaonaAddressCheckValidator(parent));
}
...
bool parseTaonaURI(const QUrl &uri, SendCoinsRecipient *out)
{
    // return if URI is not valid or is no taona: URI
    if(!uri.isValid() || uri.scheme() != QString("taona"))
        return false;
    ...
    for (QList<QPair<QString, QString> >::iterator i = items.begin(); i != items.end(); i++)
    {
        ...
        else if (i->first == "amount")
        {
            if(!i->second.isEmpty())
            {
                if(!TaonaUnits::parse(TaonaUnits::TNA, i->second, &rv.amount))
                {
                    return false;
                }
            }
...
bool parseTaonaURI(QString uri, SendCoinsRecipient *out)
{
    // Convert taona:// to taona:
    //
    //    Cannot handle this later, because taona:// will cause Qt to see the part after // as host,
    //    which will lower-case it (and thus invalidate the address).
    if(uri.startsWith("taona://", Qt::CaseInsensitive))
    {
        uri.replace(0, 10, "taona:");
    }
    QUrl uriInstance(uri);
    return parseTaonaURI(uriInstance, out);
}

QString formatTaonaURI(const SendCoinsRecipient &info)
{
    QString ret = QString("taona:%1").arg(info.address);
    int paramCount = 0;

    if (info.amount)
    {
        ret += QString("?amount=%1").arg(TaonaUnits::format(TaonaUnits::TNA, info.amount, false, TaonaUnits::separatorNever));
        paramCount++;
    }
...
bool openTaonaConf()
{
    boost::filesystem::path pathConfig = GetConfigFile(TAONA_CONF_FILENAME);
    ...
    /* Open taona.conf with the associated application */
...
fs::path static StartupShortcutPath()
{
    ...
    if (chain == CBaseChainParams::MAIN)
        return GetSpecialFolderPath(CSIDL_STARTUP) / "Taona.lnk";
    if (chain == CBaseChainParams::TESTNET) // Remove this special case when CBaseChainParams::TESTNET = "testnet4"
        return GetSpecialFolderPath(CSIDL_STARTUP) / "Taona (testnet).lnk";
    return GetSpecialFolderPath(CSIDL_STARTUP) / strprintf("Taona (%s).lnk", chain);
}
...
fs::path static GetAutostartFilePath()
{
    std::string chain = ChainNameFromCommandLine();
    if (chain == CBaseChainParams::MAIN)
        return GetAutostartDir() / "taona.desktop";
    return GetAutostartDir() / strprintf("taona-%s.lnk", chain);
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    else
    {
        ...
        if (!optionFile.good())
            return false;
        std::string chain = ChainNameFromCommandLine();
        // Write a taona.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        if (chain == CBaseChainParams::MAIN)
            optionFile << "Name=Taona\n";
        else
            optionFile << strprintf("Name=Taona (%s)\n", chain);
...
LSSharedFileListItemRef findStartupItemInList(LSSharedFileListRef list, CFURLRef findUrl)
{
    ...
    // loop through the list of startup items and try to find the taona app
...
bool GetStartOnSystemStartup()
{
    CFURLRef taonaAppUrl = CFBundleCopyBundleURL(CFBundleGetMainBundle());
    if (taonaAppUrl == nullptr) {
        return false;
    }
    
    LSSharedFileListRef loginItems = LSSharedFileListCreate(nullptr, kLSSharedFileListSessionLoginItems, nullptr);
    LSSharedFileListItemRef foundItem = findStartupItemInList(loginItems, taonaAppUrl);

    CFRelease(taonaAppUrl);
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    if(fAutoStart && !foundItem) {
        // add taona app to startup item list
        LSSharedFileListInsertItemURL(loginItems, kLSSharedFileListItemBeforeFirst, nullptr, nullptr, taonaAppUrl, nullptr, nullptr);
    }

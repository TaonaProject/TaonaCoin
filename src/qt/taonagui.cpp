...
const std::string TaonaGUI::DEFAULT_UIPLATFORM =
#if defined(Q_OS_MAC)
        "macosx"
...
const QString TaonaGUI::DEFAULT_WALLET = "~Default";

TaonaGUI::TaonaGUI(const PlatformStyle *_platformStyle, const NetworkStyle *networkStyle, QWidget *parent) :
    QMainWindow(parent),
    enableWallet(false),
    clientModel(0),
    walletFrame(0),
...
void TaonaGUI::createActions()
{
    ...
    sendCoinsAction->setStatusTip(tr("Send coins to a Taona address"));
    ...
    receiveCoinsAction->setStatusTip(tr("Request payments (generates QR codes and taona: URIs)"));
    ...
    transferAssetAction->setStatusTip(tr("Transfer assets to TNA addresses"));
    ...
    signMessageAction->setStatusTip(tr("Sign messages with your Taona addresses to prove you own them"));
    verifyMessageAction = new QAction(platformStyle->TextColorIcon(":/icons/verify"), tr("&Verify message..."), this);
    verifyMessageAction->setStatusTip(tr("Verify messages to ensure they were signed with specified Taona addresses"));
    ...
    openAction->setStatusTip(tr("Open a taona: URI or payment request"));

    showHelpMessageAction = new QAction(platformStyle->TextColorIcon(":/icons/info"), tr("&Command-line options"), this);
    showHelpMessageAction->setMenuRole(QAction::NoRole);
    showHelpMessageAction->setStatusTip(tr("Show the %1 help message to get a list with possible Taona command-line options").arg(tr(PACKAGE_NAME)));
...
void TaonaGUI::createToolBars()
{
    if(walletFrame)
    {
        ...
        label->setPixmap(QPixmap::fromImage(QImage(":/icons/taonacointext")));
...
//        toolbar->addAction(votingAction);
        ...
        QVBoxLayout* taonaLabelLayout = new QVBoxLayout(toolbarWidget);
        taonaLabelLayout->addWidget(label);
        taonaLabelLayout->addWidget(toolbar);
        taonaLabelLayout->setDirection(QBoxLayout::TopToBottom);
        taonaLabelLayout->addStretch(1);
        ...
        labelCurrentMarket->setText(tr("Taonacoin Market Price"));
...
void TaonaGUI::setClientModel(ClientModel *_clientModel)
{
...
#ifdef ENABLE_WALLET
bool TaonaGUI::addWallet(const QString& name, WalletModel *walletModel)
{
    if(!walletFrame)
        return false;
...
bool TaonaGUI::setCurrentWallet(const QString& name)
{
    if(!walletFrame)
        return false;
...
void TaonaGUI::removeAllWallets()
{
    if(!walletFrame)
        return;
    setWalletActionsEnabled(false);
...
void TaonaGUI::setWalletActionsEnabled(bool enabled)
{
...
void TaonaGUI::createTrayIcon(const NetworkStyle *networkStyle)
{
#ifndef Q_OS_MAC
...
void TaonaGUI::createTrayIconMenu()
{
#ifndef Q_OS_MAC
...
#ifndef Q_OS_MAC
void TaonaGUI::trayIconActivated(QSystemTrayIcon::ActivationReason reason)
{
...
void TaonaGUI::showDebugWindowActivateConsole()
{
    rpcConsole->setTabFocus(RPCConsole::TAB_CONSOLE);
    showDebugWindow();
}

void TaonaGUI::showHelpMessageClicked()
{
    helpMessageDialog->show();
}

#ifdef ENABLE_WALLET
void TaonaGUI::openClicked()
{
...
void TaonaGUI::gotoOverviewPage()
{
    overviewAction->setChecked(true);
    if (walletFrame) walletFrame->gotoOverviewPage();
}

void TaonaGUI::gotoHistoryPage()
{
    historyAction->setChecked(true);
    if (walletFrame) walletFrame->gotoHistoryPage();
}

void TaonaGUI::gotoReceiveCoinsPage()
{
    receiveCoinsAction->setChecked(true);
    if (walletFrame) walletFrame->gotoReceiveCoinsPage();
}

void TaonaGUI::gotoSendCoinsPage(QString addr)
{
    sendCoinsAction->setChecked(true);
    if (walletFrame) walletFrame->gotoSendCoinsPage(addr);
}

void TaonaGUI::gotoSignMessageTab(QString addr)
{
    if (walletFrame) walletFrame->gotoSignMessageTab(addr);
}

void TaonaGUI::gotoVerifyMessageTab(QString addr)
{
    if (walletFrame) walletFrame->gotoVerifyMessageTab(addr);
}

/** RVN START */
void TaonaGUI::gotoAssetsPage()
{
    transferAssetAction->setChecked(true);
    if (walletFrame) walletFrame->gotoAssetsPage();
};

void TaonaGUI::gotoCreateAssetsPage()
{
    createAssetAction->setChecked(true);
    if (walletFrame) walletFrame->gotoCreateAssetsPage();
};

void TaonaGUI::gotoManageAssetsPage()
{
...
void TaonaGUI::updateNetworkState()
{
    ...
    if (clientModel->getNetworkActive()) {
        tooltip = tr("%n active connection(s) to Taona network", "", count) + QString(".<br>") + tr("Click to disable network activity.");
    } else {
...
void TaonaGUI::setNumConnections(int count)
{
    updateNetworkState();
}

void TaonaGUI::setNetworkActive(bool networkActive)
{
    updateNetworkState();
}

void TaonaGUI::updateHeadersSyncProgressLabel()
{
...
void TaonaGUI::setNumBlocks(int count, const QDateTime& blockDate, double nVerificationProgress, bool header)
{
    if (modalOverlay)
    {
        if (header)
...
void TaonaGUI::message(const QString &title, const QString &message, unsigned int style, bool *ret)
{
    QString strTitle = tr("Taona"); // default title
...
void TaonaGUI::changeEvent(QEvent *e)
{
...
void TaonaGUI::closeEvent(QCloseEvent *event)
{
#ifndef Q_OS_MAC // Ignored on Mac
    if(clientModel && clientModel->getOptionsModel())
    {
...
void TaonaGUI::showEvent(QShowEvent *event)
{
...
#ifdef ENABLE_WALLET
void TaonaGUI::incomingTransaction(const QString& date, int unit, const CAmount& amount, const QString& type, const QString& address, const QString& label, const QString& assetName)
{
    // On new transaction, make an info balloon
    QString msg = tr("Date: %1\n").arg(date);
    if (assetName == "TNA")
        msg += tr("Amount: %1\n").arg(TaonaUnits::formatWithUnit(unit, amount, true));
    else
        msg += tr("Amount: %1\n").arg(TaonaUnits::formatWithCustomName(assetName, amount, MAX_ASSET_UNITS, true));
...
void TaonaGUI::dragEnterEvent(QDragEnterEvent *event)
{
    // Accept only URIs
...
void TaonaGUI::dropEvent(QDropEvent *event)
{
    if(event->mimeData()->hasUrls())
    {
...
bool TaonaGUI::eventFilter(QObject *object, QEvent *event)
{
...
#ifdef ENABLE_WALLET
bool TaonaGUI::handlePaymentRequest(const SendCoinsRecipient& recipient)
{
    // URI has to be valid
...
void TaonaGUI::setHDStatus(int hdEnabled)
{
...
void TaonaGUI::setEncryptionStatus(int status)
{
    switch(status)
    {
...
void TaonaGUI::showNormalIfMinimized(bool fToggleHidden)
{
    if(!clientModel)
        return;
...
void TaonaGUI::toggleHidden()
{
    showNormalIfMinimized(true);
}

void TaonaGUI::detectShutdown()
{
...
void TaonaGUI::showProgress(const QString &title, int nProgress)
{
    if (nProgress == 0)
    {
...
void TaonaGUI::setTrayIconVisible(bool fHideTrayIcon)
{
    if (trayIcon)
    {
...
void TaonaGUI::showModalOverlay()
{
    if (modalOverlay && (progressBar->isVisible() || modalOverlay->isLayerVisible()))
        modalOverlay->toggleVisibility();
}

static bool ThreadSafeMessageBox(TaonaGUI *gui, const std::string& message, const std::string& caption, unsigned int style)
{
...
void TaonaGUI::subscribeToCoreSignals()
{
...
void TaonaGUI::unsubscribeFromCoreSignals()
{
...
void TaonaGUI::toggleNetworkActive()
{
    if (clientModel) {
    ...
    menu(0)
{
    ...
    QList<TaonaUnits::Unit> units = TaonaUnits::availableUnits();
    int max_width = 0;
    const QFontMetrics fm(font());
    for (const TaonaUnits::Unit unit : units)
    {
        max_width = qMax(max_width, fm.width(TaonaUnits::name(unit)));
    }
...
{
    menu = new QMenu(this);
    for (TaonaUnits::Unit u : TaonaUnits::availableUnits())
    {
        QAction *menuAction = new QAction(QString(TaonaUnits::name(u)), this);
...
{
    setText(TaonaUnits::name(newUnits));
}
...
void TaonaGUI::getPriceInfo()
{
    request->setUrl(QUrl("https://api.binance.com/api/v1/ticker/price?symbol=TNABTC"));

...
const char* GetOpName(opcodetype opcode)
{
    ...
    /** RVN START */
    case OP_TNA_ASSET              : return "OP_TNA_ASSET";
    /** RVN END */
...
{
    if (this->size() > 30) {
        if ((*this)[25] == OP_TNA_ASSET) { // OP_TNA_ASSET is always in the 25 index of the script if it exists
            int index = -1;
            if ((*this)[27] == TNA_R) { // Check to see if TNA starts at 27 ( this->size() < 105)
                if ((*this)[28] == TNA_V)
                    if ((*this)[29] == TNA_N)
                        index = 30;
            } else {
                if ((*this)[28] == TNA_R) // Check to see if TNA starts at 28 ( this->size() >= 105)
                    if ((*this)[29] == TNA_V)
                        if ((*this)[30] == TNA_N)
                            index = 31;
            }
            ...
            if (index > 0) {
                ...
                if ((*this)[index] == TNA_T) { // Transfer first anticipating more transfers than other assets operations
                    nType = TX_TRANSFER_ASSET;
                    return true;
                } else if ((*this)[index] == TNA_Q && this->size() > 39) {
                    nType = TX_NEW_ASSET;
                    fIsOwner = false;
                    return true;
                } else if ((*this)[index] == TNA_O) {
                    nType = TX_NEW_ASSET;
                    fIsOwner = true;
                    return true;
                } else if ((*this)[index] == TNA_R) {

...
BOOST_FIXTURE_TEST_SUITE(asset_tests, BasicTestingSetup)
    ...
    BOOST_AUTO_TEST_CASE(name_validation_tests)
    {
        ...
        //- Versions of TAONACOIN NOT allowed
        BOOST_CHECK(!IsAssetNameValid("TNA", type));
        BOOST_CHECK(!IsAssetNameValid("TAONA", type));
        BOOST_CHECK(!IsAssetNameValid("TAONACOIN", type));

        //- Versions of TAONACOIN ALLOWED
        BOOST_CHECK(IsAssetNameValid("TAONA.COIN", type));
        BOOST_CHECK(IsAssetNameValid("TAONA_COIN", type));
        BOOST_CHECK(IsAssetNameValid("TNASPYDER", type));
        BOOST_CHECK(IsAssetNameValid("SPYDERTNA", type));
        BOOST_CHECK(IsAssetNameValid("TAONASPYDER", type));
        BOOST_CHECK(IsAssetNameValid("SPYDETAONA", type));
        BOOST_CHECK(IsAssetNameValid("BLACK_TAONAS", type));
        BOOST_CHECK(IsAssetNameValid("SETNAOT", type));
    ...
    BOOST_AUTO_TEST_CASE(transfer_asset_coin_test)
    {
        ...
        CAssetTransfer asset("TAONA", 1000);
    ...
    BOOST_AUTO_TEST_CASE(new_asset_coin_test)
    {
        ...
        // Create the asset scriptPubKey
        CNewAsset asset("TAONA", 1000, 8, 1, 0, "");
        CScript scriptPubKey = GetScriptForDestination(DecodeDestination(Params().GlobalBurnAddress()));
        asset.ConstructTransaction(scriptPubKey);

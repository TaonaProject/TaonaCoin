...
`test/functional/test_framework/test_framework.py:TaonaTestFramework.main`.
...
def main():
    ...
    tmpdir = "%s/taona_test_runner_%s" % (args.tmpdirprefix, datetime.datetime.now().strftime("%Y%m%d_%H%M%S"))
    os.makedirs(tmpdir)
    ...
    enable_taonad = config["components"].getboolean("ENABLE_TAONAD")

    if config["environment"]["EXEEXT"] == ".exe" and not args.force:
        # https://github.com/TaonaProject/Taonacoin/commit/d52802551752140cf41f0d9a225a43e84404d3e9
        # https://github.com/TaonaProject/Taonacoin/pull/5677#issuecomment-136646964
        print("Tests currently disabled on Windows by default. Use --force option to enable")
        sys.exit(0)

    if not (enable_wallet and enable_cli and enable_taonad):
        print("No functional tests to run. Wallet, utils, and taonad must all be enabled")
...
def run_tests(test_list, src_dir, build_dir, exeext, tmpdir, jobs=1, enable_coverage=False, args=[], combined_logs_len=0):
    # Warn if taonad is already running (unix only)
    try:
        if subprocess.check_output(["pidof", "taonad"]) is not None:
            print("%sWARNING!%s There is already a taonad process running on this system. Tests may fail unexpectedly due to resource contention!" % (BOLD[1], BOLD[0]))
    ...
    #Set env vars
    if "TAONAD" not in os.environ:
        os.environ["TAONAD"] = build_dir + '/src/taonad' + exeext
        os.environ["TAONACLI"] = build_dir + '/src/taona-cli' + exeext
...
class RPCCoverage():
    ...
    commands per `taona-cli help` (`rpc_interface.txt`).
